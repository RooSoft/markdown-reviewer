...
2. **Update the `AGENT_PROTOCOL_BLOCK` wording so it names the `.mdr` suffix and adds a cleanup step.** The block currently says:
   ...
   And add a **CLEANUP** line just after the existing `REPORT` line:
   ...
   Do **not** change the TRIAGE / APPLY / ASK / CONSISTENCY / PRESERVE semantics — only the suffix wording plus the new CLEANUP line. Keep the block a single well-formed HTML comment (no nested `-->`; the existing regression assertion in `generator.test.ts` guards this — keep it green).

   > The `.mdr` is regenerated by the server on any later annotation change, so deleting it mid-session is safe; the agent only deletes it as the final step of consuming a review. The tmpDir annotation/session data is cleaned separately via `mdr --clean`.
